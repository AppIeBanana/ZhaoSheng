    ...
    <div className="flex flex-col h-screen bg-blue-50 max-w-md mx-auto">
      ...
      <header className="bg-white shadow-sm py-2.5 px-3 border-b sticky top-0 z-10">
        <div className="flex items-center justify-between">
          ...
          >
            <i className="fa-solid fa-arrow-left text-gray-600 text-base"></i>
          </button>
          <div className="text-center flex-1">
            <h1 className="text-xs font-medium text-gray-500">智能问答系统</h1>
            <div className="flex items-center justify-center mt-0.5">
              <span className="text-blue-600 font-semibold text-xs">福州软件职业技术学院AI招生</span>
              <i className="fa-solid fa-home ml-1.5 text-gray-400 text-xs"></i>
              <i className="fa-solid fa-bars ml-2 text-gray-400 text-xs"></i>
            </div>
          </div>
          {/* 注释掉ID显示区域
          {currentDialogId && (
            <div className="bg-blue-50 px-2 py-0.5 rounded text-[10px] text-blue-600 border border-blue-100 ml-2 truncate max-w-[80px]">
              ID: {currentDialogId}
            </div>
          )}
          */}
      ...
      <div className="flex-1 overflow-y-auto pb-28">
        ...
        <div className="bg-white rounded-xl shadow-sm mx-3 mt-3 mb-4 p-3">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center">
              <div className="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center mr-1.5">
                <i className="fa-solid fa-question text-blue-600 text-[10px]"></i>
              </div>
              <h3 className="font-medium text-gray-800 text-sm">猜你想问</h3>
            </div>
            <button 
              onClick={handleChangeQuestions}
              className="text-blue-600 text-xs flex items-center"
            >
              <i className="fa-solid fa-sync-alt mr-0.5 text-xs"></i>换一批
            </button>
          </div>
          
          {/* Category tabs */}
          <div className="flex overflow-x-auto pb-1.5 mb-2.5 space-x-1 -mx-1 px-1">
            {questionCategories.map((category, index) => (
              <button
                key={index}
                onClick={() => handleCategoryChange(index)}
                className={`
                  whitespace-nowrap px-2.5 py-1 text-xs rounded-full flex-shrink-0
          ...
          <div className="space-y-1.5">
            {displayedQuestions.map((question, index) => (
              <button
                ...
                className="w-full text-left bg-blue-50 rounded-lg p-2.5 text-gray-700 hover:bg-blue-100 transition-colors flex justify-between items-center text-xs"
              >
                <span className="flex-1 pr-2">{question}</span>
                <i className="fa-solid fa-angle-right text-gray-400 text-xs flex-shrink-0"></i>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            {messages.length > 1 && (
              <div className="flex items-center justify-center my-3">
                ...
                <span className="px-2 text-[10px] text-gray-400">以上是历史记录</span>
                <div className="h-px bg-gray-200 flex-1"></div>
              </div>
            )}
            ...
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} mb-3`}>
                ...
                <div className={`max-w-[85%] ${message.sender === 'user' ? 'items-end' : 'items-start'} flex flex-col`}>
                  <div className={`
                    rounded-xl p-3 text-sm shadow-sm relative
                    ${message.sender === 'user' 
                  ...
                  {message.id !== 'welcome' && (
                    <span className={`text-[10px] mt-0.5 ${message.sender === 'user' ? 'self-end' : 'self-start'} text-gray-400`}>
      ...
      <div className="bg-white p-2.5 border-t fixed bottom-0 left-1/2 right-0 transform -translate-x-1/2 z-10 max-w-md w-full">
        <div className="relative">
           <input
            ...
            className="w-full rounded-full border border-gray-300 pl-3.5 pr-28 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            disabled={isLoading}
          />
           {/* 按钮容器 - 使用弹性布局 */}
           <div className="absolute right-2.5 top-1/2 transform -translate-y-1/2 flex space-x-1.5">
             {/* 文件上传按钮 */}
             <button 
               className="text-gray-600 hover:text-blue-600 transition-colors w-8 h-8 flex items-center justify-center"
               disabled={isLoading}
             ...
             {/* 发送按钮 */}
             <button 
               className="text-gray-600 hover:text-blue-600 transition-colors w-8 h-8 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
