                ...
                script {
                    ...
                    withCredentials([sshUserPrivateKey(credentialsId: 'jenkins_ssh', keyFileVariable: 'SSH_KEY', passphraseVariable: 'SSH_PASSPHRASE', usernameVariable: 'SSH_USERNAME')]) {
                        ...
                        withCredentials([file(credentialsId: 'zhaosheng_env', variable: 'ENV_FILE'),
                            ...
                            // 使用ssh-agent来处理带密码的私钥，并在同一脚本块中执行所有SSH/SCP命令
                            // 这样所有远程操作都能使用已加载的SSH私钥
                            sh([
                                script: '''
                                    # 启动ssh-agent
                                    eval $(ssh-agent -s)
                                    ...
                                    # 在部署服务器上加载镜像并启动服务
                                    echo '在服务器上部署应用...'
                                    ssh -o StrictHostKeyChecking=no "$SSH_USERNAME@$DEPLOY_SERVER" 'cd "$DEPLOY_PATH" && \
                                        echo "开始部署应用..." && \
                                        ...
                                        # 设置日志文件默认权限 && \
                                        find $LOG_ROOT_DIR -type f -print0 | xargs -0 chmod 644 && \
                                        # 检查设置结果 && \
                                        echo "检查日志目录设置..." && \
                                        ls -la $LOG_ROOT_DIR && \
                                        echo "日志目录设置完成！" && \
